...
if (!fs.existsSync(ADMINS_FILE))    fs.writeFileSync(ADMINS_FILE,   JSON.stringify([{ id:'admin1', name:'Admin', role:'superadmin', createdAt: new Date().toISOString() }], null, 2));
...
const loadPayments = ()  => JSON.parse(fs.readFileSync(PAYMENTS_FILE, 'utf8'));
...
function calcRevenue(tokens) {
  ...
  for (const t of Object.values(tokens)) {
    ...
    if (t.refunded) { refundTotal += t.price; continue; }
  ...
  return { total, refundTotal, netTotal: total - refundTotal, byProduct, byReseller, resellerTotal, directTotal };
}
...
app.post('/admin/generate', (req, res) => {
  const { adminKey, customerName, productId, packageLabel, price, instructionSetId, postInstructionSetId, resellerId, resellerName, autoActivate, allowDuplicate, carryOver, paymentMethod, paymentRef } = req.body;
  if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  ...
  if (!allowDuplicate) {
    ...
    const existing = Object.entries(tokens).find(([, t]) =>
      t.approved && !t.deactivated &&
      (t.customerName || '').trim().toLowerCase() === nameLower
    );
    if (existing) {
      return res.status(409).json({
        error: `A customer named "${customerName}" already has an active subscription. Confirm to add anyway.`,
        duplicate: true,
        existingToken: existing[0],
      });
    }
  }
  ...
  // autoActivate: admin activates directly, customer does not need to fill the form
  const skipCustomer = autoActivate === true || autoActivate === 'true';
  const subExpiresAt = skipCustomer
  ...
  tokens[token] = {
    ...
    credentialsMode:  skipCustomer ? true : (product.credentialsMode || false),
    ...
    durationDays,
    used:         skipCustomer,
    submittedAt:  skipCustomer ? now : undefined,
    approved:     skipCustomer,
    approvedAt:   skipCustomer ? now : undefined,
    subscriptionExpiresAt: subExpiresAt,
    subscriptionDays:      skipCustomer ? durationDays : undefined,
    // Carry-over fields from a previous link for repeated customers
    email:       (carryOver && carryOver.email)       || undefined,
    wechat:      (carryOver && carryOver.wechat)      || undefined,
    orgId:       (carryOver && carryOver.orgId)       || undefined,
    sessionData: (carryOver && carryOver.sessionData) || undefined,
    paymentMethod: paymentMethod || undefined,
    paymentRef:    paymentRef    || undefined,
    declined: false, deactivated: false,
  };
  saveTokens(tokens);
  ...
  res.json({ link, token, expiresAt, price: parseFloat(price), autoActivated: skipCustomer });
});
...
app.post('/admin/delete-customer', (req, res) => {
  const { adminKey, token } = req.body; if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  const tokens = loadTokens(); if (!tokens[token]) return res.status(404).json({ error: 'Not found.' });
  delete tokens[token]; saveTokens(tokens); res.json({ success: true });
});
...
app.get('/api/validate-token', (req, res) => {
  ...
  const { pre, post } = getInstrSets(t);
  // Include carried-over contact details so the form can pre-fill them
  res.json({ valid: true, submitted: false, customerName: t.customerName, packageType: t.packageType, product: t.product || 'claude', credentialsMode: t.credentialsMode || false,
    wechat: t.wechat || '', email: t.email || '',
    processingText: pre.processingText, approvedText: pre.approvedText, approvedSteps: pre.approvedSteps, postApprovedText: post.postApprovedText, postApprovedSteps: post.postApprovedSteps, notification: notifPayload });
});
...
app.post('/api/submit', (req, res) => {
  ...
  // Use carried-over values from token if the customer didn't re-enter them
  const effectiveWechat = (wechat && wechat.trim()) || t.wechat || '';
  const effectiveEmail  = (email  && email.trim())  || t.email  || '';
  if (!effectiveWechat)                           errors.wechat = 'WeChat ID is required.';
  if (!EMAIL_REGEX.test(effectiveEmail))          errors.email  = 'Please enter a valid email address.';
  ...
  tokens[token].wechat = effectiveWechat; tokens[token].email = effectiveEmail;
  if (!t.credentialsMode) {
...
app.post('/admin/admins/save', (req, res) => {
  ...
  if (idx > -1) list[idx] = { ...list[idx], ...admin };
  else          list.push({ id: uuidv4(), createdAt: new Date().toISOString(), ...admin });
  saveAdmins(list); res.json({ success: true });
});
app.post('/admin/admins/delete', (req, res) => {
  const { adminKey, id } = req.body;
  if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  const list = loadAdmins().filter(a => a.id !== id);
  saveAdmins(list); res.json({ success: true });
});
...
app.post('/admin/payments/save', (req, res) => {
  const { adminKey, payment } = req.body;
  if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  const list = loadPayments();
  const idx  = list.findIndex(p => p.id === payment.id);
  if (idx > -1) list[idx] = { ...list[idx], ...payment };
  else          list.push({ id: uuidv4(), createdAt: new Date().toISOString(), ...payment });
  savePayments(list); res.json({ success: true });
});
// Refund a payment and mark associated token
app.post('/admin/payments/refund', (req, res) => {
  const { adminKey, paymentId, refundedBy, note } = req.body;
  if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  const payments = loadPayments();
  const tokens   = loadTokens();
  ...
  payments[pidx] = { ...p, status: 'refunded', refundedAt: new Date().toISOString(), refundedBy: refundedBy || 'admin', refundNote: note || '' };
  savePayments(payments);
  // Also mark the token as refunded
  if (p.token && tokens[p.token]) {
    tokens[p.token].refunded = true;
    tokens[p.token].refundedAt = new Date().toISOString();
    tokens[p.token].refundedBy = refundedBy || 'admin';
    tokens[p.token].refundPaymentMethod = p.paymentMethod || '';
    saveTokens(tokens);
  }
  res.json({ success: true });
});

// ── Edit token (subscription mid-flight) ─────────────────────────────────────
app.post('/admin/token/edit', (req, res) => {
  const { adminKey, token, customerName, email, wechat, orgId, sessionData, subscriptionExpiresAt, packageType, price, paymentMethod } = req.body;
  if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  const tokens = loadTokens();
  if (!tokens[token]) return res.status(404).json({ error: 'Not found.' });
  const editable = { customerName, email, wechat, orgId, sessionData, subscriptionExpiresAt, packageType, price: price ? parseFloat(price) : undefined, paymentMethod };
  Object.entries(editable).forEach(([k, v]) => { if (v !== undefined && v !== null && v !== '') tokens[token][k] = v; });
  tokens[token].lastEditedAt = new Date().toISOString();
  saveTokens(tokens);
  res.json({ success: true });
});

// ── Customer list (all, grouped by name) ────────────────────────────────────
app.get('/admin/customers/all', (req, res) => {
  if (!isAdmin(req.query.adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  const tokens = loadTokens();
  // Group tokens by customerName — each unique name = one customer entry with all their subscriptions
  const grouped = {};
  for (const [tok, t] of Object.entries(tokens)) {
    if (!t.approved && !t.declined) continue; // skip pending/unused
    const key = (t.customerName || '').toLowerCase().trim();
    if (!grouped[key]) grouped[key] = { name: t.customerName, email: t.email, wechat: t.wechat, subscriptions: [] };
    grouped[key].subscriptions.push({ token: tok, ...t });
  }
